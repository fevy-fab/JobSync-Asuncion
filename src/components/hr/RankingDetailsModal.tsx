                        ...
                        {(() => {
                          ...
                          if (skillMatchPairs.length > 0) {
                            ...
                            if (strongMatches === totalRequired) {
                              return (
                                <span className="text-green-600">
                                  Fully matches all {totalRequired} required skills. ⭐
                                </span>
                              );
                            }

                            // No full matches, only related skills
                            if (strongMatches === 0 && relatedOnly > 0) {
                              return (
                                <span className="text-orange-600">
                                  No required skills are fully matched yet, but {relatedOnly} related
                                  skills contribute to the overall {applicant.skillsScore.toFixed(1)}% skills score. 🔸
                                </span>
                              );
                            }

                            // Mixture: some full matches + some related
                            return (
                              <span className="text-yellow-600">
                                Fully matches {strongMatches} of {totalRequired} required skills,
                                plus {relatedOnly} related skill{relatedOnly > 1 ? 's' : ''} that
                                help build the {applicant.skillsScore.toFixed(1)}% skills score. ⚡
                              </span>
                            );
                          }

                          // Fallback: if for some reason we don't have match pairs, use backend count
                          const matchedSkillsCount = applicant.matchedSkillsCount ?? 0;
                          ...
                          if (matchedSkillsCount === 0) {
                            if (applicant.skillsScore > 0) {
                              return (
                                <span className="text-orange-600">
                                  No required skills are fully matched yet, but several related skills
                              ...
                              {skillMatchPairs.map((pair, idx) => {
                                ...
                                const relationLabel =
                                  pair.matchType === 'exact'
                                    ...
                                    ? 'keyword overlap / related skill'
                                    : null;

                                const isStrongMatch =
                                  pair.matchType === 'exact' || pair.matchType === 'high';
                                const isRelatedOnly =
                                  pair.matchType === 'medium' || pair.matchType === 'token';
                                ...
                                return (
                                  ...
                                  >
                                    {/* Icon now distinguishes FULL vs RELATED vs NO match */}
                                    {hasSkill ? (
                                      isStrongMatch ? (
